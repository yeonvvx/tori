...
function createTray() {
    ...
    const contextMenu = Menu.buildFromTemplate([{
        ...
        id: "quit", label: "Quit Tori", click: () => {
            cleanupAndExit()
        },
    },
    ])

    tray.setToolTip("Tori")
    ...
    return new Promise<void>((resolve, reject) => {
        ...
        if (_development) {
            ...
            if (process.env.TEST_DATADIR) {
                ...
                const devDataDir = path.join(app.getPath("appData"), "Tori-dev")
        ...
        proc.on("close", (code: number | null) => {
            ...
            if (!startupResolved && !isShutdown) {
                ...
                if (crashScreen && !crashScreen.isDestroyed()) {
                    crashScreen.show()
                    crashScreen.webContents.send("crash", `Tori server process terminated with status: ${code}. Closing in 10 seconds.`)
...
app.whenReady().then(async () => {
    ...
    try {
        ...
        dialog.showErrorBox("Tori Denshi could not start MpvCore", message)
        app.quit()
        return
    }
    if (_development) {
