...
func addQuitItem() {
	...
	mQuit := systray.AddMenuItem("Quit Tori", "Quit the whole app")
	mQuit.Enable()
	go func() {
		<-mQuit.ClickedCh
...
func onReady(webFS *embed.FS, app *core.App, flags core.SeanimeFlags, selfupdater *updater.SelfUpdater) func() {
	return func() {
		...
		systray.SetTitle(fmt.Sprintf("Tori v%s", constants.Version))
		systray.SetTooltip(fmt.Sprintf("Tori v%s", constants.Version))
		log.Trace().Msg("systray: App is ready")

		// Menu items
		systray.AddMenuItem("Tori v"+constants.Version, "Tori version")
